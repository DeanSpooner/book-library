    ...
    describe('with no records in the database', async () => {
        ...
        describe('POST /authors', () => {
            it('creates a new author in the database', async () => {
                ...
                expect(response.body.author).to.equal('Ryu Murakami');
                expect(authorRecord.author).to.equal('Ryu Murakami');
    ...
    describe('with records in the database', async () => {
        ...
        describe('POST /authors', () => {
            it('returns a 409 error if a matching title and author already exists', async () => {
                ...
                expect(response.body.error).to.equal(`The author Lewis Carroll already exists.`);
            });
        });
        ...
        describe('GET /authors/:id', () => {
            ...
            it('returns a 404 if the author does not exist', async () => {
                const response = await request(app).get('/authors/1985');
        ...
        describe('DELETE /authors/:id', () => {
            it('deletes author record by id', async () => {
                const author = authors.filter(
                    (n) => n.dataValues.author === 'Lewis Carroll'
                )[0].dataValues;
